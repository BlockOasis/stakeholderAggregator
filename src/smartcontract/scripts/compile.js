...
function findOpenZeppelinContract(importPath) {
  ...
  const fullPath = path.resolve(__dirname, "../../../node_modules/@openzeppelin/contracts", importPath);
...
let compiledContract = JSON.parse(solc.compile(JSON.stringify(compilerInput), { import: findImports }));
...
const compiledData = {
  ...
  bytecode: compiledContract.contracts["BlockOasisAggregator.sol"][contractName].evm.bytecode.object,
